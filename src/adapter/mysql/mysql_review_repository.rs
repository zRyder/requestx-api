...
use crate::adapter::mysql::{
    model::{prelude, review},
};
use crate::adapter::mysql::model::prelude::Review;
...
impl<'a> ReviewRepository for MySqlReviewRepository<'a> {
    async fn create_record(self, record: review::ActiveModel) -> Result<InsertResult<review::ActiveModel>, DbErr> {
        Review::insert(record).exec(self.db_conn).await
    }

    async fn get_record(&self, level_id: u64, discord_id: u64) -> Result<Option<review::Model>, DbErr> {
        Review::find_by_id((level_id, discord_id)).one(self.db_conn).await
    }
