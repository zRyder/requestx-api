    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            LevelReviewError::DatabaseError(db_err) => {
                write!(f, "Unable to submit review due to database error: {}", db_err)
            }
            LevelReviewError::LevelRequestDoesNotExist => {
                write!(f, "Unable to create level request: Level request does not exist")
            }
        }
    }
}
...
impl Into<LevelReviewApiResponseError> for LevelReviewError {
    fn into(self) -> LevelReviewApiResponseError {
        match self {
            ...
            LevelReviewError::LevelRequestDoesNotExist => {LevelReviewApiResponseError::LevelRequestDoesNotExist}
        }
    }
}